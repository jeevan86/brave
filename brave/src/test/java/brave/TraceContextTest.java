...
public class TraceContextTest {
  ...
  @Test public void compareUnequalIds() {
    TraceContext id = TraceContext.newBuilder().traceId(333L).spanId(0L).build();
  ...
  @Test public void compareEqualIds() {
    TraceContext id = TraceContext.newBuilder().traceId(333L).spanId(444L).build();
  ...
  @Test public void equalsOnlyAccountsForIdFields() {
    TraceContext id = TraceContext.newBuilder().traceId(333L).spanId(444L).debug(true).build();

    assertThat(id)
        .isEqualTo(TraceContext.newBuilder().traceId(333L).spanId(444L).sampled(true).build());
  }
  ...
  @Test public void hashCodeOnlyAccountsForIdFields() {
    ...
    assertThat(id.hashCode())
        .isEqualTo(
            TraceContext.newBuilder().traceId(333L).spanId(444L).sampled(true).build().hashCode());
  }

  @Test
  public void testToString_lo() {
    TraceContext id = TraceContext.newBuilder().traceId(333L).spanId(3).parentId(2L).build();
